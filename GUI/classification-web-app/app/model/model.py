...
class ParallelCheXNetSwin(nn.Module):
    def __init__(self, num_classes=14, img_size=224):
        super().__init__()
        self.img_size = img_size
        self.num_classes = num_classes
        ...
        print(f"Initializing Swin Transformer model structure")
        ...
        print(f"Swin backbone feature dimension: {self.swin_feature_dim}")



        # Fusie methode: Concatenatie
        self.swin_project_for_fusion = nn.Identity() 
        fusion_output_dim = self.cnn_feature_dim + self.swin_feature_dim # Dimensie na concat - 2048

        print(f"CNN output dimension: {self.cnn_feature_dim}")
        print(f"Swin output dimension: {self.swin_feature_dim}")
        print(f"Fusion output dimension: {fusion_output_dim} (for concat)")


        # MLP Classifier
        # Input dimensie is de fusie output dimensie (1024 bij sommatie)
        self.classifier = nn.Sequential(
            nn.Linear(self.cnn_feature_dim + self.swin_feature_dim, 1024),
            nn.ReLU(True),
            nn.Dropout(0.5),
            nn.Linear(1024, num_classes),
            # Sigmoid niet nodig door BCEWithLogitsLoss (zit ingebouwd)
        )
    ...
    def forward(self, x):
        ...
        fused_feat = torch.cat((cnn_feat, swin_feat_projected), dim=1)
    
        # Classifier forward (de input dimensie van de eerste Linear laag moet je aanpassen naar cnn_feature_dim + swin_feature_dim)
        output = self.classifier(fused_feat)
        return output


# Initialiseer model zonder CheXNet- en SwinTransformer-gewichten
model = ParallelCheXNetSwin(
    num_classes=14,  # labels
    img_size=224  # input size
)
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = model.to(device)

# Laad eigen gewichten
custom_weights_path = 'C:/Users/ricks/MAAI_scriptie/UI/classification-web-app/app/model/weights/best_aurocV2.pth'  # Pas dit pad aan naar je eigen gewichtenbestand  C:\Users\ricks\MAAI_scriptie\UI\classification-web-app\app\model\weights\best_aurocV2.pth
if os.path.isfile(custom_weights_path):
    try:
        print(f"Loading custom trained weights from {custom_weights_path}...")
        custom_checkpoint = torch.load(custom_weights_path, map_location=torch.device('cpu'), weights_only=False)
        model.load_state_dict(custom_checkpoint['model_state_dict'])
        print("Custom trained weights successfully loaded.")
    except Exception as e:
        print(f"Error loading custom trained weights: {e}")
else:
    print(f"Warning: Custom weights file not found at {custom_weights_path}. Model will use default initialization.")
